<template>
  <div>
    <section class="content">
      ...
      <el-table :data="carCollisions" v-loading="listLoading" class="center-block" stripe
        ...
        <el-table-column align="left" label="分析目标" prop="followType" min-width="100"
    ...
    methods: {
      ...
      cancelCollis() {
        ...
        this.$confirm('确认要取消关联该交并任务吗?', '提示', {type: 'info'}).then(() => {
          this.$post('/car/cancelCase/' + this.caseId, arr, '取消关联成功').then((data) => {
            if ("000000" === data.code) {
              this.getData();
              this.sels = [];
            }
      ...
      getData() {
        ...
        this.$post('/car/task/query', this.query).then((data) => {
          this.carCollisions = data.data.list;
