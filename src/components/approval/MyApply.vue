      ...
      <el-table :data="imsiList" v-loading="listLoading" class="center-block" stripe>
        ...
        <el-table-column align="left" label="操作" width="160">
          <template slot-scope="scope">
            <el-button type="text" @click="cancel(scope.row)" v-show="activeItem=='EXECUTION'">撤销</el-button>
...
<script>
  export default {
    ...
    methods: {
      handleType(val) {
        if (val.name === 'FINISH') {
          this.timeColumn = [{label: '终止时间', prop: 'time', min: 170, max: 170},
            {label: '终止原因', prop: 'reason', min: 120, max: 200}]
        } else if (val.name === 'HANDLED') {
          this.timeColumn = [{label: '完成时间', prop: 'time', min: 170, max: 170}]
        } else {
          this.timeColumn = [];
        }
      },
      ...
      gotoDetail(row) {
        if (this.activeItem === 'EXECUTION') {
          this.$router.push({path: '/approvalDetail', query: {type: 1}})
        } else {
